    ...
    <div style={{ marginBottom: 12 }}>
      ...
      <input
        ...
        style={{
          ...
          fontSize: 13, fontFamily: FONT, color: "#0f172a",
    ...
    <div style={{ marginBottom: 10 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <div style={{ flex: 1 }}>
          ...
          <input
            type="number" value={m}
            onChange={e => onChange(Number(e.target.value), Number(e.target.value) * 12)}
            style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <input
            type="number" value={a}
            onChange={e => onChange(Math.round(Number(e.target.value) / 12), Number(e.target.value))}
            style={{
              width: "100%", border: "1.5px solid #e2e8f0", borderRadius: 6,
              padding: "8px 10px", fontSize: 12, fontFamily: FONT,
              outline: "none", background: "#fff"
...
function OfferLetterDoc({ d }) {
  ...
  const page = {
    ...
    fontSize: "11pt", color: "#1a1a1a",
    lineHeight: 1.6, padding: "14mm 16mm 20mm",
    position: "relative", boxSizing: "border-box", pageBreakBefore: "always", breakBefore: "page",
  };
  const secTitle = {
    fontSize: "10.5pt", fontWeight: 800, color: NAVY,
    letterSpacing: "0.05em", textTransform: "uppercase",
    borderBottom: `2px solid ${BLUE}`, paddingBottom: 4,
    marginBottom: 10, marginTop: 18, fontFamily: FONT,
  };
  const subHead = { fontSize: "10.5pt", fontWeight: 700, color: NAVY, marginBottom: 3, marginTop: 12, fontFamily: FONT };
  const bodyP = { fontSize: "10pt", marginBottom: 8, textAlign: "justify", fontFamily: FONT };
  const tblWrap = { width: "100%", borderCollapse: "collapse", marginBottom: 10, fontSize: "10pt", fontFamily: FONT };
  const th = { background: NAVY, color: "#fff", padding: "7px 10px", fontWeight: 700, textAlign: "left", fontSize: "9.5pt", fontFamily: FONT };
  const thR = { ...th, textAlign: "right" };
  const tdL = { padding: "6px 10px", fontWeight: 700, color: NAVY, background: "#eef2f8", borderBottom: "1px solid #dde3ed", width: "36%", fontSize: "9.5pt", verticalAlign: "top", fontFamily: FONT };
  const tdV = { padding: "6px 10px", borderBottom: "1px solid #dde3ed", fontSize: "9.5pt", verticalAlign: "top", fontFamily: FONT };
  const tdN = { padding: "6px 10px", borderBottom: "1px solid #dde3ed", textAlign: "right", fontSize: "9.5pt", fontFamily: FONT };
  const tblBorder = "1px solid #c8d3e6";

  const footer = (pg) => (
    <div style={{
      position: "absolute", bottom: 8, left: 16, right: 16,
      borderTop: "1px solid #dde3ed", paddingTop: 5,
      display: "flex", justifyContent: "space-between",
      fontSize: "8pt", color: "#94a3b8", fontFamily: FONT
    }}>
  ...
  return (
    <div id="ol-document" style={{ background: "#fff" }}>

      {/* PAGE 1: COVER */}
      <div style={{ ...page, padding: 0, minHeight: "297mm", display: "flex", flexDirection: "column", pageBreakBefore: "auto", breakBefore: "auto" }}>
        <div style={{ background: NAVY, height: "28%", width: "100%", flexShrink: 0 }} />
        <div style={{
          flex: 1, display: "flex", flexDirection: "column",
          alignItems: "center", justifyContent: "center",
          padding: "10mm 20mm", textAlign: "center"
        }}>
          <div style={{ fontSize: "22pt", fontWeight: 800, color: NAVY, letterSpacing: "0.1em", marginBottom: 4, fontFamily: FONT }}>EJECT SOLUTIONS</div>
          <div style={{ fontSize: "10.5pt", color: BLUE, fontStyle: "italic", marginBottom: 20, fontFamily: FONT }}>Empowering Digital Innovation</div>
          <div style={{ width: 240, height: 2, background: BLUE, marginBottom: 20 }} />
          <div style={{ fontSize: "17pt", fontWeight: 800, color: NAVY, letterSpacing: "0.12em", marginBottom: 4, fontFamily: FONT }}>OFFER OF EMPLOYMENT</div>
          <div style={{ fontSize: "10.5pt", color: BLUE, fontStyle: "italic", marginBottom: 20, fontFamily: FONT }}>Letter of Appointment</div>
          <div style={{ width: 240, height: 2, background: BLUE, marginBottom: 28 }} />
          <div style={{ fontSize: "9.5pt", color: "#64748b", marginBottom: 6, fontFamily: FONT }}>Prepared for:</div>
          <div style={{ fontSize: "17pt", fontWeight: 800, color: NAVY, marginBottom: 3, fontFamily: FONT }}>{d.candidate.fullName}</div>
          <div style={{ fontSize: "10.5pt", color: BLUE, marginBottom: 20, fontFamily: FONT }}>{d.candidate.designation}</div>
          <div style={{ fontSize: "9.5pt", color: "#64748b", fontFamily: FONT }}>Date of Issue: {d.issueDate}</div>
          <div style={{ fontSize: "9.5pt", color: "#64748b", fontFamily: FONT }}>Date of Joining: {d.joiningDate}</div>
        </div>
        <div style={{
          background: NAVY, height: 50, width: "100%",
          display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0
        }}>
          <span style={{ color: "#fff", fontSize: "9pt", fontWeight: 700, letterSpacing: "0.18em", fontFamily: FONT }}>CONFIDENTIAL | EJECT SOLUTIONS</span>
        </div>
      </div>
      ...
      <div style={{ ...page }}>
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: "16pt", fontWeight: 800, color: NAVY, letterSpacing: "0.08em", fontFamily: FONT }}>EJECT SOLUTIONS</div>
          <div style={{ fontSize: "9.5pt", color: BLUE, fontStyle: "italic", fontFamily: FONT }}>Empowering Digital Innovation</div>
          <div style={{ fontSize: "9pt", color: "#64748b", fontFamily: FONT }}>www.ejectsolutions.com | {d.hrEmail}</div>
          <div style={{ height: 1, background: "#c8d3e6", margin: "8px 0" }} />
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "9pt", fontFamily: FONT }}>
        ...
        <table style={{ ...tblWrap, border: tblBorder }}>
          <tbody>
            {[
              ["Full Name", d.candidate.fullName],
              ["Designation", d.candidate.designation],
              ["Department", d.candidate.department],
              ["Date of Joining", d.joiningDate],
              ["Employment Type", d.candidate.employmentType],
              ["Work Location", d.candidate.workLocation],
              ["Reporting To", d.candidate.reportingTo],
              ["Mobile", d.candidate.mobile],
      ...
      <div style={{ ...page }}>
        ...
        <table style={{ ...tblWrap, border: tblBorder }}>
          <thead>
            <tr>
              ...
              <th style={thR}>Monthly (INR)</th>
              <th style={thR}>Annual (INR)</th>
            </tr>
          </thead>
          <tbody>
            {[
              ["Basic Salary", comp.basic],
              ["House Rent Allowance (HRA)", comp.hra],
              ["Special Allowance", comp.special],
              ["Medical Allowance", comp.medical],
              ["Transport / Fuel Allowance", comp.transport],
            ].map(([label, c]) => (
              <tr key={label}>
                <td style={{ ...tdV, width: "50%" }}>{label}</td>
                <td style={tdN}>{INR(c.monthly)}</td>
                <td style={tdN}>{INR(c.annual)}</td>
              </tr>
            ))}
            <tr>
        ...
        <ul style={{ paddingLeft: 18, fontSize: "10pt", lineHeight: 1.65, marginBottom: 8, fontFamily: FONT }}>
          {[
            "Design, develop, and deploy Generative AI applications leveraging LLMs including OpenAI, Gemini, AWS Bedrock, and Groq.",
            "Build and maintain RAG (Retrieval Augmented Generation) pipelines with vector databases (ChromaDB, Qdrant) for enterprise knowledge retrieval.",
            "Develop agentic AI systems using LangChain and LangGraph, including multi-step ReAct agent orchestration.",
            "Build scalable backend APIs using FastAPI (Python) and Node.js/Express.js for AI-driven product features.",
            "Develop responsive frontend interfaces using React.js and Next.js, consuming AI and backend services.",
            "Deploy and manage containerized microservices on AWS (EKS, Lambda, RDS, S3, ElastiCache) with CI/CD pipelines via GitHub Actions.",
            "Collaborate with product, design, and data science teams to translate business requirements into production-grade AI solutions.",
            "Contribute to code reviews, technical documentation, and continuous improvement of engineering standards.",
            "Stay current with the rapidly evolving GenAI landscape and propose innovative solutions.",
          ].map((item, i) => (
      ...
      <div style={{ ...page }}>
        ...
        <table style={{ ...tblWrap, border: tblBorder }}>
          <thead>
            <tr>
              <th style={{ ...th, width: "35%" }}>Benefit</th>
              <th style={th}>Details</th>
            </tr>
          </thead>
          <tbody>
            {[
              ["Health Insurance", "Group medical insurance coverage of INR 5 Lakhs for employee and immediate family (spouse & 2 children)"],
              ["Annual Leave", "18 days of paid annual leave per calendar year, accrued monthly"],
              ["Sick Leave", "10 days of paid sick leave per calendar year"],
              ["Public Holidays", "As per Government of Telangana and national holiday calendar"],
              ["Learning & Development", "Annual budget of INR 30,000 for certifications, courses, and conferences"],
              ["Performance Bonus", "Annual performance-linked bonus subject to company and individual performance (to be communicated separately)"],
              ["Flexible Working", "Hybrid model with flexible timing as per team and project requirements"],
              ["Team Events", "Quarterly team outings and annual company offsite"],
            ].map(([k, v]) => (
              <tr key={k}><td style={tdL}>{k}</td><td style={tdV}>{v}</td></tr>
            ))}
          </tbody>
      ...
      <div style={{ ...page }}>
        ...
        <ol style={{ paddingLeft: 20, fontSize: "10pt", lineHeight: 1.7, marginBottom: 20, fontFamily: FONT }}>
          {[
            ...
            "B.Tech / Graduation degree certificate and all semester mark sheets (KL University, 2022)",
            "Previous employment offer letter(s), appointment letter(s), and experience letter(s)",
            "Last 3 months' salary slips from most recent employer",
            "Bank account details (cancelled cheque / bank passbook copy)",
            "No Objection Certificate (NOC) from previous employer, if applicable",
            "Any certifications relevant to your role (Full Stack, Node.js, or others)",
          ].map((item, i) => (
            <li key={i}>{item}</li>
          ))}
        </ol>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 24, marginBottom: 16, fontFamily: FONT }}>
          <tbody>
            <tr>
              <td style={{ width: "50%", verticalAlign: "top", paddingRight: 28 }}>
                <div style={{ fontWeight: 700, fontSize: "10.5pt", color: NAVY, marginBottom: 36, fontFamily: FONT }}>For Eject Solutions</div>
                <div style={{ borderTop: "1px solid #334155", width: 180, paddingTop: 5 }}>
                  <div style={{ fontSize: "10pt", fontFamily: FONT }}>Authorised Signatory</div>
                  <div style={{ fontSize: "9pt", color: "#64748b", fontFamily: FONT }}>Human Resources | Eject Solutions</div>
                </div>
              </td>
              <td style={{ width: "50%", verticalAlign: "top" }}>
                <div style={{ fontWeight: 700, fontSize: "10.5pt", color: NAVY, marginBottom: 36, fontFamily: FONT }}>Accepted by Candidate</div>
                <div style={{ borderTop: "1px solid #334155", width: 180, paddingTop: 5 }}>
                  <div style={{ fontSize: "10pt", fontFamily: FONT }}>{d.candidate.fullName}</div>
                  <div style={{ fontSize: "9pt", color: "#64748b", fontFamily: FONT }}>Date: ____________________</div>
                </div>
              </td>
            </tr>
          </tbody>
        </table>
        <p style={{ fontSize: "10pt", textAlign: "center", color: "#334155", marginTop: 14, fontFamily: FONT }}>Place: Hyderabad, Telangana, India</p>
        <div style={{ height: 1, background: "#c8d3e6", margin: "14px 0 8px" }} />
        <p style={{ fontSize: "8.5pt", color: "#94a3b8", textAlign: "center", fontStyle: "italic", fontFamily: FONT }}>
          This is a computer-generated document. For queries, contact {d.hrEmail} | This document is strictly confidential.
        </p>
        {footer(6)}
      </div>
    </div>
  ...
  return (
    <>
      <style>{`
        ...
        /* Salary totals box */
        .salary-total {
          background: ${NAVY}; color: #fff; border-radius: 7px;
            ...
            <div className="editor-content">
              ...
              {tab === "comp" && <>
                ...
                {[
                  ...
                  <SalaryRow
                    key={key} label={label}
                    m={data.compensation[key].monthly}
                    a={data.compensation[key].annual}
                    onChange={(m, a) => setComp(key, m, a)}
                  />
                ))}
                <div className="salary-total">
                  Monthly: ₹{INR(totalM)}<br />
                  Annual CTC: ₹{INR(totalA)}
                </div>
