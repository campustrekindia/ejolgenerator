...
import { LOGO_B64, SIG_B64 } from "./imgs.js";
...
/* ── Department master with codes ── */
const DEPARTMENTS = [
  { code: "ENG-AI",  label: "Engineering & AI Products" },
  { code: "ENG-WEB", label: "Web & Frontend Engineering" },
  { code: "ENG-BE",  label: "Backend & Infrastructure" },
  { code: "DATA",    label: "Data Science & Analytics" },
  { code: "PROD",    label: "Product Management" },
  { code: "DESIGN",  label: "Design & UX" },
  { code: "MKT",     label: "Marketing & Growth" },
  { code: "SALES",   label: "Sales & Business Development" },
  { code: "OPS",     label: "Operations & Administration" },
  { code: "FIN",     label: "Finance & Accounts" },
  { code: "HR",      label: "Human Resources" },
  { code: "LEGAL",   label: "Legal & Compliance" },
];

/* Generate employee code: ES-{DEPTCODE}-{YEAR}-{SEQ} */
const makeEmpCode = (deptCode, year, seq) =>
  `ES-${deptCode}-${year}-${String(seq).padStart(4, "0")}`;
  ...
  const Letterhead = () => (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
        <img src={LOGO_B64} alt="Eject Solutions" style={{ height: 44, objectFit: "contain" }} />
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 9, color: "#64748b", fontFamily: F }}>www.ejectsolutions.com</div>
          <div style={{ fontSize: 9, color: "#64748b", fontFamily: F }}>{d.hrEmail}</div>
        </div>
      </div>
      <div style={{ height: 1, background: "#c8d3e6", margin: "6px 0" }} />
      ...
      <div style={{ height: 1, background: "#c8d3e6", margin: "6px 0 14px" }} />
    </div>
  );
    ...
    <div id="ol-pages">
      ...
      <div data-pdf-page="1" style={{ ...pageStyle, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 80px", textAlign: "center" }}>
          {/* Logo image */}
          <img src={LOGO_B64} alt="Eject Solutions" style={{ width: 220, marginBottom: 20, objectFit: "contain" }} />
      ...
      <div data-pdf-page="2" style={contentPage}>
        ...
        <table style={{ ...tblWrap, border: tblBorder }}>
          <tbody>
            {[
              ["Employee Code", makeEmpCode(d.candidate.deptCode, d.empYear, d.empSeq)],
              ["Full Name", d.candidate.fullName],
              ["Designation", d.candidate.designation],
              ["Department", `${d.candidate.department} (${d.candidate.deptCode})`],
        ...
        <table style={{width:"100%",borderCollapse:"collapse",marginTop:24,marginBottom:16,fontFamily:F}}>
          <tbody><tr>
            <td style={{width:"50%",verticalAlign:"top",paddingRight:28}}>
              <div style={{fontWeight:700,fontSize:11,color:NAVY,marginBottom:8,fontFamily:F}}>For Eject Solutions</div>
              {/* Signature & stamp image */}
              <img src={SIG_B64} alt="Authorised Signatory" style={{ height: 64, objectFit: "contain", display: "block", marginBottom: 0 }} />
              <div style={{borderTop:"1px solid #334155",width:200,paddingTop:5}}>
                <div style={{fontSize:10,fontFamily:F,fontWeight:700,color:NAVY}}>Authorised Signatory</div>
                <div style={{fontSize:9,color:"#64748b",fontFamily:F}}>Director — Eject Solutions Pvt Ltd</div>
              </div>
            </td>
            <td style={{width:"50%",verticalAlign:"top"}}>
              <div style={{fontWeight:700,fontSize:11,color:NAVY,marginBottom:8,fontFamily:F}}>Accepted by Candidate</div>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .editor-content select {
          ...
          background: var(--ui-navy); cursor: pointer;
        ...
        .emp-code-label {
          font-size: 9px; font-weight: 700; color: var(--ui-yellow);
          ...
          <div className={`editor-panel${mobileView === "editor" ? " mobile-visible" : ""}`}>
            ...
            <div className="editor-content">
              {tab === "basic" && <>
                ...
                <div style={{ marginTop: 8, padding: "10px 12px", background: "#0d1b2e", borderRadius: 6, border: "1px solid #1e3050" }}>
                  <div style={{ fontSize: 9, color: "#7a8fa6", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>Generated Employee Code</div>
                  <div style={{ fontSize: 13, fontWeight: 800, color: "#f5c518", letterSpacing: "0.08em", fontFamily: "monospace" }}>
                    {makeEmpCode(data.candidate.deptCode, data.empYear, data.empSeq)}
                  </div>
                  <div style={{ fontSize: 9, color: "#7a8fa6", marginTop: 3 }}>Edit in Candidate tab → Dept / Year / Seq</div>
                </div>
              </>}
              {tab === "candidate" && <>
                ...
                {/* Department dropdown */}
                <div style={{ marginBottom: 12 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "#f5c518", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 4, fontFamily: "monospace" }}>Department</div>
                  <select
                    value={data.candidate.deptCode}
                    onChange={e => {
                      const dept = DEPARTMENTS.find(d => d.code === e.target.value);
                      setData(d => ({ ...d, candidate: { ...d.candidate, deptCode: dept.code, department: dept.label } }));
                    }}
                  >
                    {DEPARTMENTS.map(dept => (
                      <option key={dept.code} value={dept.code}>{dept.code} — {dept.label}</option>
                    ))}
                  </select>
                </div>
                ...
                <div style={{ marginBottom: 6 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "#f5c518", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 4, fontFamily: "monospace" }}>Employee Code</div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 9, color: "#7a8fa6", marginBottom: 3, textTransform: "uppercase", letterSpacing: "0.06em" }}>Year</div>
                      <input value={data.empYear} onChange={e => setData(d => ({ ...d, empYear: e.target.value }))}
                        style={{ width: "100%", padding: "8px 10px", border: "1.5px solid #1e3050", borderRadius: 6, fontSize: 12, fontFamily: "'Montserrat',sans-serif", color: "#e8edf5", outline: "none", background: "#0d1b2e" }}
                        onFocus={e => { e.target.style.borderColor = "#f5c518"; }} onBlur={e => { e.target.style.borderColor = "#1e3050"; }}
                      />
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 9, color: "#7a8fa6", marginBottom: 3, textTransform: "uppercase", letterSpacing: "0.06em" }}>Seq No</div>
                      <input value={data.empSeq} onChange={e => setData(d => ({ ...d, empSeq: e.target.value }))}
                        placeholder="0013"
                        style={{ width: "100%", padding: "8px 10px", border: "1.5px solid #1e3050", borderRadius: 6, fontSize: 12, fontFamily: "'Montserrat',sans-serif", color: "#e8edf5", outline: "none", background: "#0d1b2e" }}
                        onFocus={e => { e.target.style.borderColor = "#f5c518"; }} onBlur={e => { e.target.style.borderColor = "#1e3050"; }}
                      />
